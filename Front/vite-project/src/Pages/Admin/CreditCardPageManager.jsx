...
import s from "./CreditCardPageManager.module.scss";

const CreditCardPageManager = () => {
  ...
  const handleImageUpload = async (e, target, itemId = null, itemType = null) => {
    ...
    if (imageUrl) {
      ...
      } else if (target === "benefit" && itemId) {
        updateBenefit(itemId, { ...benefits.find(b => b.id === itemId), image_url: imageUrl });
      } else if (target === "step" && itemId) {
        updateStep(itemId, { ...steps.find(s => s.id === itemId), icon_url: imageUrl });
      }
  ...
  const addStep = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/admin/credit-card-steps",
        {
          title: "Новый шаг",
          description: "Описание шага",
          icon: String(steps.length + 1),
          order_position: steps.length + 1,
          is_active: true,
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className={s.pageManager}>
      <div className={s.header}>
        ...
        <h2>Управление страницей "Кредитная карта"</h2>
      ...
      <div className={s.tabs}>
        <button
          className={`${s.tab} ${activeTab === "banner" ? s.activeTab : ""}`}
          onClick={() => setActiveTab("banner")}
        >
          Баннер
        </button>
        <button
          className={`${s.tab} ${activeTab === "benefits" ? s.activeTab : ""}`}
          onClick={() => setActiveTab("benefits")}
        >
          Преимущества
        </button>
        <button
          className={`${s.tab} ${activeTab === "steps" ? s.activeTab : ""}`}
          onClick={() => setActiveTab("steps")}
        >
          Шаги
        </button>
        <button
          className={`${s.tab} ${activeTab === "form" ? s.activeTab : ""}`}
          ...
          <div className={s.formGroup}>
            ...
            <div className={s.uploadArea}>
              <input
                ...
                placeholder="/images/credit-card-banner.png"
              />
            ...
            {banner.image_url && (
              <div className={s.imagePreview}>
                <img src={banner.image_url} alt="Баннер" />
                <button 
      ...
      {activeTab === "benefits" && (
        <div className={s.section}>
          <div className={s.blockHeader}>
            <h3>Преимущества</h3>
            <button onClick={addBenefit} className={s.addBtn}>
              + Добавить
            </button>
          </div>
              ...
              <div key={b.id} className={s.benefitCard}>
                ...
                <div className={s.formGroup}>
                  ...
                  <input
                    ...
                    value={b.title}
                    onChange={(e) =>
                      updateBenefit(b.id, { ...b, title: e.target.value })
                    }
                  />
                </div>
                <div className={s.formGroup}>
                  ...
                  <div className={s.uploadArea}>
                    <input
                      type="text"
                      value={b.image_url}
                      onChange={(e) => updateBenefit(b.id, { ...b, image_url: e.target.value })}
                    ...
                    <div className={s.uploadButtons}>
                      <label className={s.uploadBtn}>
                        ...
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleImageUpload(e, "benefit", b.id)}
                          style={{ display: "none" }}
                        />
                      </label>
                    </div>
                  ...
                  {b.image_url && (
                    <div className={s.imagePreview}>
                      ...
                      <button 
                        className={s.removeImageBtn}
                        onClick={() => updateBenefit(b.id, { ...b, image_url: "" })}
                      >
                        ✕
                      </button>
                    </div>
                ...
                <label className={s.checkbox}>
                  <input
                    ...
                    checked={b.is_active}
                    onChange={(e) =>
                      updateBenefit(b.id, { ...b, is_active: e.target.checked })
                    }
                  />{" "}
                  Активен
                </label>
      ...
      {activeTab === "steps" && (
        <div className={s.section}>
          <div className={s.blockHeader}>
            <h3>Шаги оформления</h3>
            <button onClick={addStep} className={s.addBtn}>
              + Добавить шаг
            </button>
          </div>
          <div className={s.stepsGrid}>
            {steps.map((step) => (
              <div key={step.id} className={s.stepCard}>
                <div className={s.cardHeader}>
                  <h4>
                    Шаг {step.icon || step.id}: {step.title}
                  </h4>
                  <button
                ...
                <div className={s.formGroup}>
                  ...
                  <input
                    ...
                    value={step.title}
                    onChange={(e) =>
                      updateStep(step.id, { ...step, title: e.target.value })
                    }
                  />
                </div>
                <div className={s.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateStep(step.id, { ...step, description: e.target.value })
                    }
                  />
                </div>
                <div className={s.formGroup}>
                  ...
                  <input
                    ...
                    value={step.icon}
                    onChange={(e) =>
                      updateStep(step.id, { ...step, icon: e.target.value })
                    }
                  />
                </div>
                <div className={s.formGroup}>
                ...
                <label className={s.checkbox}>
                  <input
                    ...
                    onChange={(e) =>
                      updateStep(step.id, { ...step, is_active: e.target.checked })
                    }
                  />{" "}
                  Активен
                </label>
...
export default CreditCardPageManager;